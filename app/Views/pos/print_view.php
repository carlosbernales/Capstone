            ...
            <div class="row">
              <div class="col-lg-6">
                <div class="invoice-title">
                  ...
                  <form method="post" action="<?= site_url('insert-data'); ?>" enctype="multipart/form-data">
                ...
                <div class="invoice-number d-flex justify-content-between">
                    <h6>REF_ID. <span><?= $reference_id; ?></span></h6><input type="hidden" name="reference_id" value="REF_ID. <?= $reference_id; ?>">
                    <address>
                      <strong>Date:</strong>
                      <?= date('Y-m-d'); ?></span><br>
                    </address>
                      ...
                      <table class="table table-striped table-hover table-md">
                        ...
                        <?php foreach ($formDataArray as $index => $formData) : ?>
                          <tr>
                            ...
                            <td><span><?= $formData['price']; ?></span><input type="hidden" name="formDataArray[<?= $index; ?>][price]" value="<?= $formData['price']; ?>"></td>
                            <td><span><?= $formData['new_input']; ?></span><input type="hidden" name="formDataArray[<?= $index; ?>][new_input]" value="<?= $formData['new_input']; ?>"></td>
                            <td><span><?= $formData['total_input']; ?></span><input type="hidden" name="formDataArray[<?= $index; ?>][total_input]" value="<?= $formData['total_input']; ?>"></td>
                         
                            <td class="hidden-td">
                            <span><?= $formData['product_id']; ?></span>
                            <input type="hidden" name="formDataArray[<?= $index; ?>][product_id]" value="<?= $formData['product_id']; ?>">
                            </td>
                          </tr>
                        <?php endforeach; ?>
                      </table>
              ...
              <div class="col-lg-12 text-md-right">
                <div class="float-lg-left mb-lg-0 mb-3 hide-on-print">
                  ...
                  <a href="<?= base_url('pos_view') ?>" class="btn btn-danger btn-icon icon-left"><i class="fas fa-times"></i><span>Cancel</span></a>
                </div>
              </div>
            </div>
            </form>
