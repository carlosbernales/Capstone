          ...
          <div class="card">
            <div class="card-body">
              <div class="table-responsive">
                <form action="<?php echo site_url('submit-form'); ?>" method="post" id="my-form">
                  <datalist id="product-list">
                    <?php foreach ($product as $productName) : ?>
                      <?php if ($productName['category_id'] == '<category_id>') : ?>
                        <option value="<?php echo $productName['product_name']; ?>" data-category="<?php echo $productName['category_id']; ?>" data-price="<?php echo $productName['product_price']; ?>" data-image="<?php echo $productName['product_image']; ?>" data-id="<?php echo $productName['id']; ?>"></option>
                      <?php endif; ?>
                    <?php endforeach; ?>
                  </datalist>
  ...
  function displayProducts(selectedCategoryId) {
    ...
    products.forEach(function(product) {
        if (selectedCategoryId === '' || product.category_id == selectedCategoryId) {
            ...
            var cardContent = `
                <div class="card">
                    <img src="public/public/products/${product.product_image}" class="card-img-top p-2 rounded product-image" width="100" height="200" alt="Product Image" data-product='${JSON.stringify(product)}'>
                    <div class="card-body">
...
document.addEventListener('click', function (event) {
  if (event.target.classList.contains('product-image')) {
    ...
    var existingRow = document.querySelector('.repeater-row input[name="product_id[]"][value="' + productId + '"]');
    if (existingRow) {
    ...
    var productImage = productData.product_image;
    var productName = productData.product_name;
    var productPrice = productData.product_price;
    var productId = productData.id;
    ...
    imageInput.setAttribute('src', 'public/public/products/' + productImage);
    ...
    priceInput.classList.add('input', 'form-control', 'col-md-2', 'ml-1');
    ...
    removeBtnContainer.appendChild(productIdInput);
...
function calculateSum() {
  ...
  var sum = 0;
  rows.forEach(function (row) {
    var quantity = parseInt(row.querySelector('input[name="new_input[]"]').value) || 0;
    var price = parseFloat(row.querySelector('input[name="price[]"]').value) || 0;
    sum += quantity * price;
  });

  document.getElementById('receipt-total').textContent = 'Total: ' + sum.toFixed(2);
  updateChange(); // Update the change value when the sum changes
}
