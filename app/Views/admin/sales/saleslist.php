...
<head>
  ...
  <script>
    function printDataTable() {
        ...
        // Your signature with an image on top
        var signatureText = "Owner Signature";
        var lineLength = signatureText.length * 10; // Adjust the multiplier for proper length
        var signature = `<br><div style='display: flex; flex-direction: column; align-items: center;'><img alt="image" src="<?= base_url('public/assets/img/signature.png') ?>" class="header-logo" width="150" height="100" /><div style='width: ${lineLength}px; padding-bottom: 5px; text-align: center;'><h4 style='border-top: 1px solid black;'>${signatureText}</h4></div></div>`;

        // Concatenate the signature to the footer
        var combinedFooter = footer + signature;
        ...
        printWindow.document.write(combinedFooter);
...
<div class="loader"></div>
                    ...
                    <div class="table-responsive">
                      <table class="table table-striped table-hover" id="example" style="width:100%;">
                        <thead>
                          ...
                          <th>Order Date</th>
                          <th>Total</th>
                          <th class="hide-on-print">View</th> <!-- Hide this column on print -->
                          </tr>
                        </thead>
                        <tbody>
                        <?php foreach ($completed_orders as $orders): ?>
                          ...
                          <td><?=$orders['order_date_formatted']?></td>
                          <td><?= $orders['order_amount'] + $orders['labor'] ?></td>
                          <td style = "width:13px;"><button class="btn btn-success btn-sm hide-on-print" data-toggle="modal" data-target="#editModal-<?php echo $orders['id']; ?>" data-id="<?php echo $orders['id']; ?>" style="width: 70px;">View</button></td>
                          <?php $orderItems = $Order_Item_model->where('fk_order_id', $orders['id'])->findAll(); ?>
                          </tr>
                        ...
                        <!-- EDIT CUSTOM MODAL -->
                        <div class="modal fade" id="editModal-<?php echo $orders['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true" data-backdrop="false">
                          <div class="modal-dialog modal-dialog-centered modal-md" role="document">
                            <div class="modal-content" >
                              <div class="modal-header">
                                <h5 class="modal-title" id="editModalLabel-<?php echo $orders['id']; ?>">Sale Details</h5>
                              ...
                              <div class="modal-body">
                                <div class="row">
                                  ...
                                  foreach ($orderItems as $orderItem) {
                                    ...
                                    <div class="col-6">
                                      <?= csrf_field(); ?>
                                      <div class="mb-3 form-group d-flex align-items-start order-item" style="position: relative; border: 1px solid #000; padding: 10px; border-radius: 5px;">
                                        <div class="ml-3">
                                          ...
                                          <label for="item_name" class="form-label">&#8369;<?= $itemAmount ?></label>
                                        </div>
                                        <div class="order-amount" style="position: absolute; bottom: 10px; right: 10px;">
                              ...
                              <div class="modal-footer">
                                <div class="mb-3 form-group text-right">
                                  <div>
                                    <?php
                                    if (!empty($orders['labor']) && $orders['labor'] !== '0') {
                                      echo "Labor: &#8369;{$orders['labor']}<br>";
                                      echo "Total: &#8369;{$orders['order_amount']}<br>";
                                    }
                                    ?>
                                    Subtotal: &#8369;<?= $orders['order_amount'] + $orders['labor'] ?><br>
                                  </div>
                                </div>
                              </div>
                            </div>
  ...
  <!-- Custom JS File -->
  <script src="<?= base_url('public/assets/js/custom.j') ?>s"></script>
